...
const DomainBannerSlider = ({ instituteSlug }) => {
    ...
    const toTitleCase = (slug) => {
        const titleCase = slug
            ...
            .join(' ');
        console.log("Converted title case:", slug, "to", titleCase);
        return titleCase;
    };
    ...
    const fetchBannerList = async () => {
        try {
            ...
            if (response && response.banners) {
                const pathSlug = window.location.pathname.split('/')[1]; // e.g. "vg-study-hub"

                // Filter banners based on domain name matching the slug
                const filteredBanners = response.banners.filter(banner =>
                    banner.active === true &&
                    banner.domains?.some(domain => toSlug(domain.name) === pathSlug)
                );
...
export default DomainBannerSlider;
